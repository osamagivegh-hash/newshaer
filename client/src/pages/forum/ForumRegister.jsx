...
const ForumRegister = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const res = await axios.post('/api/forum-auth/register', formData);
            if (res.data.success) {
                forumLogin(res.data.user, res.data.token);
                navigate('/family-tree/forum');
            }
        } catch (err) {
            setError(err.response?.data?.message || 'فشل عملية التسجيل');
        } finally {
            setLoading(false);
        }
    };
    ...
    return (
        <div className="max-w-md mx-auto bg-white rounded-xl shadow-md p-8 mt-10" dir="rtl">
            <h2 className="text-2xl font-bold text-center text-palestine-green mb-6">تسجيل عضوية جديدة</h2>
            ...
            <p className="mt-4 text-center text-gray-600">
                لديك حساب بالفعل؟ <Link to="/family-tree/forum/login" className="text-palestine-blue font-bold">تسجيل الدخول</Link>
            </p>
        </div>
    );
};
